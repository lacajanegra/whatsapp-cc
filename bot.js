...
var token = '139100738:AAHSVXLb3Q3sS30pSaQZ8xJGTIgAQf1KDKs';
...
console.log(">> mensaje " + msg.text + " recibido desde id:" + msg.chat.id);
   ...
   console.log("Running");
    ...
    receiveMessages(msg.chat.id, msg.text);
});
                    ...
                    console.log("> nuevo agente, id: " + message.id + " nombre: " + message.name);
                     ...
                     var firstOfWaitingStack = popWaitingStack();
                      if (firstOfWaitingStack == -1) {
                        ...
                        agents.push(agent);
                        FSmDirect.push({name: firstOfWaitingStack[0], text: firstOfWaitingStack[0]+' esta en conversación contigo', to: "agent", num: firstOfWaitingStack[0] , id:message.id});
  ...
  }else
  {
    var flagFromMethod=1;
      ...
      FSnWaitingStack.orderByChild("num").equalTo(pop[0]).on("child_added", function(snapshot) {
                    ...
                    FSmDirect.push({name: data.num, text: data.num + " esta en conversación contigo", to: "agent", num: data.num });
...
function sendMsg(num,text){
   Alice.sendMessage( num,text, undefined, undefined, '');    
